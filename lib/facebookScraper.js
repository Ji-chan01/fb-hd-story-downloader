  ...
  while (true) {
    ...
    for (const group of arr) {
      ...
      for (const rep of reps) {
        ...
        const isAudio = rep.mime_type.includes('audio') ||
          (rep.codecs && (rep.codecs.startsWith('mp4a') || rep.codecs.startsWith('ac-3')));
        ...
        const qualityLabel =
          ...
          height >= 540 ? '540p' : height >= 480 ? '480p' :
          height >= 360 ? '360p' : (height ? `${height}p` : 'unknown');
        ...
        if (isVideo) {
          if (!clipKey) {
            ...
            try { clipKey = new URL(rep.base_url).pathname.split('/').slice(0, 8).join('/'); }
            catch { clipKey = rep.base_url.slice(0, 60); }
          }
      ...
      if (clip.videoQualities.length > 0 && clipKey && !seenClipKeys.has(clipKey)) {
...
async function extractPhotoUrlsFromPage(page) {
  ...
  const domPhotos = await page.evaluate(() => {
    return Array.from(document.querySelectorAll('img[src*="fbcdn.net"]'))
      .map(img => img.src);
  }).catch(() => []);
  ...
  const isExcluded = (url) =>
    ...
    url.includes('s40x40') || url.includes('s75x75') ||
    url.includes('s160x160') || url.includes('s320x320') ||
    url.includes('s235x350') || url.includes('s480x270') ||
    url.includes('/p40x40/') || url.includes('/p160x160/') ||
    ...
    url.includes('/emoji') || url.includes('sticker');

  const allPhotos = [...domPhotos, ...htmlPhotos].filter(u => u && !isExcluded(u));
  return allPhotos;
}
...
async function scrapeFacebookStoryMedia(targetInput, cUser, xsCookie) {
  ...
  try {
    ...
    await page.goto(initialNavUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
...
async function captureStoryFromEmbeddedData(page, target, storyUrl) {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    await page.evaluate(() => {
      ...
      (document.querySelector('div[role="dialog"]') || document.body).dispatchEvent(evt);
  ...
  const profileDetails = await page.evaluate(() => {
    ...
    for (const link of links) {
      ...
      if (href && !href.includes("/stories/") && !href.includes("/sharer/") && !href.includes("/pages/") && link.innerText) {
        const text = link.innerText.trim();
        if (text && !text.includes("\n") && text.length > 1 && text.length < 50) {
          const lower = text.toLowerCase();
          if (lower !== "home" && lower !== "stories" && lower !== "create story" && lower !== "facebook" && lower !== "watch") {
            name = text;
            break;
          }
        }
    ...
    if (!name) {
      const heading = document.querySelector("h2") || document.querySelector("h1") || document.querySelector("strong");
    ...
    for (const img of imgs) {
      if (img.src && (img.src.includes("fbcdn.net") || img.src.includes("scontent"))) {
        const alt = (img.alt || "").toLowerCase();
        if (alt.includes("profile") || alt.includes("avatar") || img.width === 32 || img.width === 40 || img.height === 32 || img.height === 40) {
          avatar = img.src;
          break;
        }
      }
    ...
    if (!avatar && imgs.length > 0) {
      const smallImg = imgs.find(img => img.src && (img.src.includes("fbcdn") || img.src.includes("scontent")) && img.width < 100 && img.width > 20);
  ...
  let finalName = (profileDetails && profileDetails.name) || displayName || target.usernameOrId;
  if (!finalName || finalName.toLowerCase() === "create stories" || finalName.toLowerCase() === "create story" || finalName.toLowerCase() === "stories") {
    finalName = target.usernameOrId;
  }

  const finalAvatar = (profileDetails && profileDetails.avatar) || `https://api.dicebear.com/7.x/identicon/svg?seed=${target.usernameOrId}`;

  const videoCount = items.filter(i => i.type === "video").length;
  const photoCount = items.filter(i => i.type === "image").length;
  console.log(`[Story Scraper] Done — ${items.length} items (${videoCount} videos, ${photoCount} photos).`);
 ...
 */
async function extractSlideMedia(page, items, capturedVideoKeys, capturedPhotoMap) {
  ...
  for (const clip of clips) {
    ...
    const videoCount = items.filter(i => i.type === 'video').length;
    console.log(`[Story Scraper] [video] clip#${videoCount + 1} — ${clip.videoQualities.map(q => q.tag).join(', ')}`);
    items.push({
