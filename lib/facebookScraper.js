...
async function scrapeFacebookStoryMedia(targetInput, cUser, xsCookie) {
  ...
  if (!cUser || !xsCookie)
    throw new Error(
      "Session not available. Please contact System Administrator",
    );
  ...
  try {
    ...
    // Block Facebook's story seen/read receipt mutations to keep the dummy account invisible
    ...
    page.on("request", (request) => {
      try {
        ...
        if (
          ...
          (postData.includes("StoriesUpdateSeenStateMutation") ||
            postData.toLowerCase().includes("seen_state") ||
            postData.toLowerCase().includes("seenstate"))
        ) {
          console.log("[Story Scraper] Intercepted and blocked seen receipt mutation.");
          request.abort().catch(() => {});
        } else {
    ...
    // Evaluate if we are logged out in the page context
    const isLoggedOutInDOM = await page.evaluate(() => {
      if (document.querySelector('input[name="email"]') || document.querySelector('form[action*="/login"]')) {
        return true;
      }
      if (window.__user === "0" || window.__user === 0) {
        return true;
      }
      const html = document.documentElement.innerHTML;
      if (html.includes('"CurrentUserInitialData"') && html.includes('"USER_ID":"0"')) {
        return true;
      }
      return false;
    }).catch(() => false);
