...
async function scrapeFacebookStoryMedia(targetInput, cUser, xsCookie) {
  ...
  if (!cUser || !xsCookie)
    throw new Error(
      "Session not available. Please contact System Administrator or try again later.",
    );
  ...
  try {
    ...
    page.on("request", (request) => {
      try {
        ...
        if (
          ...
          console.log(
            "[Story Scraper] Intercepted and blocked seen receipt mutation.",
          );
          request.abort().catch(() => {});
        } else {
    ...
    const isLoggedOutInDOM = await page
      .evaluate(() => {
        if (
        ...
        if (
          html.includes('"CurrentUserInitialData"') &&
